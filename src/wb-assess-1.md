...
```
console.log is used to show the output of whatever code is being produced in the console, used to see if functions are working correctly. While return calls back a previously made value to be used.
```

**_What's the difference between defining and calling a function?_**

```
Defining is when you state the name of a function, before it even does anything. Calling is when you take a previously defined variable and actually use it.
```
...
```
A string of text that can include both letters and numbers. Can be used in functions, shows what you would expect the console to print
```

**_Number_**

```
Can be used to set maximum and minimum in different loops
```

**_Boolean_**

```
A value than can only output as true or false/yes or no.
```

**_Array_**

```
An array is an ordered list of items in the form of strings, and can be edited/overhauled in multiple different ways.
```
